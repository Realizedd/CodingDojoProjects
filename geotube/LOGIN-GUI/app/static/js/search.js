...
var API_ACCESS_KEY = 'AIzaSyAOjm6iXMyWLQqWKBsOdO5AUtHfCv0RgNY';
...
function handleClientLoad() {
    gapi.client.load('youtube', 'v3', function() {
        $.getScript('https://maps.googleapis.com/maps/api/js?sensor=false&callback=handleMapsLoad&libraries=places&key=' + API_ACCESS_KEY);
    });
}
...
function loadParamsFromURL() {
    ...
    if (startURL && startURL.indexOf('?q=') > 0) {
        ...
        console.log(paramListCollection);
...
function initializeMap(inputLat, inputLong) {
    ...
    for (var i = 0; i < finalResults2.length; i++) {
        ...
        var image = {
            url: 'static/images/redMarker_' + imageNumber + '.png',
...
function getLocationSearchResults() {
    geocoder.geocode({
        'address': inputObject.inputSearchLocation
...
$(document).ready(function() {
    ...
    $('#favorite_switch').click(function(){
        $('#loginAndFavourites').css('visibility','visible');
